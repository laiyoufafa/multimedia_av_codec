...
- 播放，音效渲染
  ...
  播放音频，对音频进行音效处理之前，需要先对音频进行解码，然后送到硬件，或者音效

- 音频编辑

  音频的编辑是针对PCM来进行的，所以需要首先对音频进行解码出PCM，然后进行编辑操作
...
``` c++
//通过 codecname 创建解码器
OH_AVCodec *audioDec = OH_AudioDecoder_CreateByName("OH.Media.Codec.Decoder.Audio.Mpeg");

//通过 codecname 创建解码器
OH_AVCodec *audioDec = OH_AudioDecoder_CreateByMime("audio/mpeg");
```
...
2. 设置回调函数(必须)
   ...
   // 设置 OnError 回调函数
       ...
       // 配置异步回调
       int32_t ret = OH_AudioDecoder_SetCallback(audioDec, cb, userData);
       if (ret != AV_ERR_OK) {
           // 异常处理
       }
   ```
