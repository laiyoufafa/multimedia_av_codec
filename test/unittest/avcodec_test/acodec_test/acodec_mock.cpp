...
int32_t ACodecMock::StartDec()
{
    ...
    if (inputLoopDec_ == nullptr) {
        ...
        UNITTEST_CHECK_AND_RETURN_RET_LOG(inputLoopDec_ != nullptr, AVCS_ERR_OK, "Fatal: No memory");
    }
...
void ACodecMock::InputFuncDec()
{
    ...
    while (true) {
        ...
        if (PushInputDataDecInner(index, bufferSize) != AVCS_ERR_OK) {
            acodecSignal_->errorNum_ += 1;
        } else {
            decInCnt_ ++;
        }
...
int32_t ACodecMock::StartEnc()
{
    ...
    if (inputLoopEnc_ == nullptr) {
        ...
        UNITTEST_CHECK_AND_RETURN_RET_LOG(inputLoopEnc_ != nullptr, AVCS_ERR_OK, "Fatal: No memory");
    }

    if (outputLoopEnc_ == nullptr) {
        outputLoopEnc_ = make_unique<thread>(&ACodecMock::OutputFuncEnc, this);
        UNITTEST_CHECK_AND_RETURN_RET_LOG(outputLoopEnc_ != nullptr, AVCS_ERR_OK, "Fatal: No memory");
    }
...
int32_t ACodecMock::PushInputDataEncInner()
{
    ...
    UNITTEST_CHECK_AND_RETURN_RET_LOG(bufferEnc != nullptr, AVCS_ERR_INVALID_VAL, "Fatal: GetEncInputBuffer fail");
    ...
    if (flagDecOut == 1) {
        ...
        if (memcpy_s(bufferEnc->GetAddr(), bufferEnc->GetSize(), bufferDec->GetAddr(), sizeDecOut) != AVCS_ERR_OK) {
        ...
        if (audioDec_->FreeOutputData(indexDec) != AVCS_ERR_OK) {
...
void ACodecMock::InputFuncEnc()
{
    ...
    while (true) {
        ...
        if (PushInputDataEncInner() != AVCS_ERR_OK) {
...
void ACodecMock::OutputFuncEnc()
{
    ...
    while (!isEncOutputEOS_) {
        ...
        if (encOutflag == 1) {
            ...
            if (audioEnc_->FreeOutputData(index) != AVCS_ERR_OK) {
                cout << "Fatal: FreeOutputData fail" << endl;
                acodecSignal_->errorNum_ += 1;
            }
        }
