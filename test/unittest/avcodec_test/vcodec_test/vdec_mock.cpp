...
int32_t VDecMock::Start()
{
    ...
    UNITTEST_CHECK_AND_RETURN_RET_LOG(inputLoop_ != nullptr, AVCS_ERR_OK, "Fatal: No memory");

    outputLoop_ = make_unique<thread>(&VDecMock::OutLoopFunc, this);
    UNITTEST_CHECK_AND_RETURN_RET_LOG(outputLoop_ != nullptr, AVCS_ERR_OK, "Fatal: No memory");
...
void VDecMock::InpLoopFunc()
{
    ...
    while (true) {
        ...
        if (PushInputDataMock(index, bufferSize) != AVCS_ERR_OK) {
...
void VDecMock::OutLoopFunc()
{
    ...
    while (true) {
        ...
        if (index != EOS_INDEX && videoDec_->RenderOutputData(index) != AVCS_ERR_OK) {
